...
const AdminReport = () => {
  const [reportType, setReportType] = useState<'managerial'>('managerial'); // فقط managerial نگه می‌داریم
  const [managerialData, setManagerialData] = useState({
  ...
  useEffect(() => {
    const fetchManagerialData = async () => {
      try {
        ...
        console.log('Received managerial report data:', data);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-purple-100 to-purple-200 p-6">
      ...
      <div className="flex justify-center gap-4 mb-8">
        <button
          onClick={() => setReportType('managerial')}
          className={`relative px-6 py-3 rounded-lg text-white font-semibold text-lg transition-all duration-300 ${
            reportType === 'managerial' ? 'bg-purple-600' : 'bg-purple-400'
          } hover:bg-purple-700 focus:outline-none focus:ring-4 focus:ring-purple-300 animate-glow`}
          style={{
            animation: 'glow 1.5s ease-in-out infinite alternate',
            boxShadow: '0 0 15px rgba(107, 33, 168, 0.7)',
          }}
        >
          گزارشات مدیریتی
          <style>
            {`
              @keyframes glow {
                from {
                  box-shadow: 0 0 10px rgba(107, 33, 168, 0.5), 0 0 20px rgba(107, 33, 168, 0.3);
                }
                to {
                  box-shadow: 0 0 20px rgba(107, 33, 168, 0.8), 0 0 30px rgba(107, 33, 168, 0.5);
                }
              }
            `}
          </style>
        ...
        <button
          ...
          className={`relative px-6 py-3 rounded-lg text-white font-semibold text-lg transition-all duration-300 ${
            reportType === 'managerial' ? 'bg-emerald-400' : 'bg-emerald-600' // استایل برعکس شد چون هدایت می‌کنیم
          } hover:bg-emerald-700 focus:outline-none focus:ring-4 focus:ring-emerald-300 animate-glow`}
          style={{
            animation: 'glow-emerald 1.5s ease-in-out infinite alternate',
